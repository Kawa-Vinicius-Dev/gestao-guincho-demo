...
insert into public.despesas
 (descricao,categoria_id,valor,data_lancamento,motorista_id,veiculo_id,natureza,status,aprovada,protocolo,criado_por,data_pagamento)
values
 ('Almoço de Fulano',2,50,'2026-09-10',1,null,'ALIMENTACAO_FUNCIONARIO','PAGO',true,null,'aaaaaaaa-0000-0000-0000-000000000001','2026-09-15'),
 ('Pedágio da viagem',3,30,'2026-09-11',1,1,'GERAL','PAGO',true,null,'aaaaaaaa-0000-0000-0000-000000000001','2026-09-15'),
 ('Diesel do plantão',1,200,'2026-09-12',1,1,'GERAL','PENDENTE',false,null,'aaaaaaaa-0000-0000-0000-000000000001',null),
 ('Multa rejeitada',3,90,'2026-09-13',1,null,'GERAL','REJEITADO',false,null,'aaaaaaaa-0000-0000-0000-000000000001',null),
 ('Almoço de agosto',2,40,'2026-08-10',1,null,'ALIMENTACAO_FUNCIONARIO','PAGO',true,null,'aaaaaaaa-0000-0000-0000-000000000001','2026-09-15'),
 ('Comissão paga',1,700,'2026-09-20',1,null,'GERAL','PAGO',true,'COMISSAO-1','aaaaaaaa-0000-0000-0000-000000000001','2026-09-20');
...
select pg_temp.checar('lista as despesas da janela da OP, e so elas',
  (select jsonb_array_length(public.detalhe_socorrista_op(1,1) -> 'despesas')::text), '3');
select pg_temp.checar('a mais recente vem primeiro',
  (public.detalhe_socorrista_op(1,1) -> 'despesas' -> 0 ->> 'descricao'), 'Diesel do plantão');
select pg_temp.checar('despesa de outro periodo fica fora',
  (select count(*)::text from jsonb_array_elements(public.detalhe_socorrista_op(1,1) -> 'despesas') e
...
select pg_temp.checar('a alimentação é marcada como desconto',
  (select e ->> 'descontaDaComissao' from jsonb_array_elements(public.detalhe_socorrista_op(1,1) -> 'despesas') e
    where e ->> 'descricao' = 'Almoço de Fulano'), 'true');
select pg_temp.checar('o pedágio aparece sem descontar',
  (select e ->> 'descontaDaComissao' from jsonb_array_elements(public.detalhe_socorrista_op(1,1) -> 'despesas') e
...
select pg_temp.checar('o desconto da comissão continua sendo só a alimentação',
  (public.detalhe_socorrista_op(1,1) -> 'comissao' ->> 'alimentacaoAprovada'), '50.00');
select pg_temp.checar('a despesa com viatura aparece com a viatura',
  (select e ->> 'veiculo' from jsonb_array_elements(public.detalhe_socorrista_op(1,1) -> 'despesas') e
    where e ->> 'descricao' = 'Pedágio da viagem'), 'L168');
select pg_temp.checar('socorrista sem despesa recebe lista vazia, não erro',
  (select jsonb_array_length(public.detalhe_socorrista_op(2,1) -> 'despesas')::text), '0');
