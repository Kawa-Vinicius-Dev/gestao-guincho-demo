...
class PortoFluxoFinanceiroApiIntegrationTest {
    ...
    private long criarUsuario(String token,String nome,String email) throws Exception {String resposta=mvc.perform(post("/api/usuarios").header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"nome\":\""+nome+"\",\"email\":\""+email+"\",\"senha\":\"Socorrista@123\",\"perfil\":\"FUNCIONARIO\"}" )).andExpect(status().isCreated()).andReturn().getResponse().getContentAsString();return ((Number)JsonPath.read(resposta,"$.id")).longValue();}
