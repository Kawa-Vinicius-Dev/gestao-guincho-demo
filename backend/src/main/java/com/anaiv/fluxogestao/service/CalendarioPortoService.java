...
public class CalendarioPortoService {
    ...
    private void validar(CalendarioRequest request,Long id){validarDataUnica(request.dataPagamento(),id);PeriodoQuinzena periodo=periodo(request.competenciaInicio());if(!periodo.inicio().equals(request.competenciaInicio())||!periodo.fim().equals(request.competenciaFim()))throw new IllegalArgumentException("O período de competência deve corresponder a uma quinzena completa.");repositorio.findByCompetenciaInicioAndCompetenciaFim(request.competenciaInicio(),request.competenciaFim()).filter(x->!x.getId().equals(id)).ifPresent(x->{throw new IllegalArgumentException("Já existe um pagamento Porto configurado para esta quinzena.");});}
