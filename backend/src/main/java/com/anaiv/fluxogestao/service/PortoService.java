...
public class PortoService {
    ...
    private final PendenciaFinanceiraPortoRepository pendencias;private final JustificativaConciliacaoPortoRepository justificativas;private final UsuarioRepository usuarios;private final CalendarioPagamentoPortoRepository calendario;private final CalendarioPortoService calendarioService;
    public PortoService(OrdemPagamentoPortoRepository ops,OrdemServicoPortoRepository oss,PendenciaFinanceiraPortoRepository p,JustificativaConciliacaoPortoRepository j,UsuarioRepository u,CalendarioPagamentoPortoRepository calendario,CalendarioPortoService calendarioService){this.ops=ops;this.oss=oss;pendencias=p;justificativas=j;usuarios=u;this.calendario=calendario;this.calendarioService=calendarioService;}
    ...
    @Transactional public OrdemPagamentoResponse criarOpManual(OrdemPagamentoRequest request){if(ops.findByNumero(request.numero().trim()).isPresent())throw new IllegalArgumentException("Já existe uma ordem de pagamento com este número.");
        OrdemPagamentoPorto op=new OrdemPagamentoPorto(request.numero().trim(),null);aplicarManual(op,request);return op(ops.save(op));}
    @Transactional public OrdemPagamentoResponse atualizarOpManual(Long id,OrdemPagamentoRequest request){OrdemPagamentoPorto op=obterOp(id);ops.findByNumero(request.numero().trim()).filter(x->!x.getId().equals(id)).ifPresent(x->{throw new IllegalArgumentException("Já existe uma ordem de pagamento com este número.");});aplicarManual(op,request);return op(op);}
