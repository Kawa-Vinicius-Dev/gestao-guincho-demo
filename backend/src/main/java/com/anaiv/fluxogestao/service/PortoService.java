...
public class PortoService {
    ...
    public AcaoLinhaPorto classificarOsGeral(LinhaPorto linha,OrdemServicoPorto os){if(os==null)return AcaoLinhaPorto.IMPORTAR;
        boolean mudou=diferente(os.getValorTotal(),linha.decimal("valor_total"))||diferenteSeInformado(os.getEspecialidade(),linha.texto("especialidade"))||diferenteSeInformado(os.getSiglaViatura(),linha.texto("sigla_viatura"))||diferenteSeInformado(os.getSocorrista(),linha.texto("socorrista"))||diferenteSeInformado(os.getQra(),linha.texto("qra"))||!Objects.equals(os.getDataAtendimento(),linha.data("data_atendimento"));return mudou?AcaoLinhaPorto.DIVERGENCIA:AcaoLinhaPorto.IGNORAR;}
    public AcaoLinhaPorto classificarAguardando(LinhaPorto linha,OrdemServicoPorto os){if(os==null)return AcaoLinhaPorto.IMPORTAR;boolean conflito=diferentePreenchido(os.getValorTotal(),linha.decimal("valor_total"))||conflita(os.getEspecialidade(),linha.texto("especialidade"))||conflita(os.getSocorrista(),linha.texto("socorrista"))||conflita(os.getQra(),linha.texto("qra"))||conflita(os.getPrestador(),linha.texto("prestador"))||conflita(os.getSeguradora(),linha.texto("seguradora"))||conflita(os.getCliente(),linha.texto("cliente"))||conflita(os.getPlaca(),linha.texto("placa"))||(os.getDataHoraAtendimento()!=null&&linha.dataHora("data_atendimento")!=null&&!os.getDataHoraAtendimento().isEqual(linha.dataHora("data_atendimento")));if(conflito)return AcaoLinhaPorto.DIVERGENCIA;boolean completa=(os.getEspecialidade()==null&&linha.texto("especialidade")!=null)||(os.getSocorrista()==null&&linha.texto("socorrista")!=null)||(os.getQra()==null&&linha.texto("qra")!=null)||(os.getPrestador()==null&&linha.texto("prestador")!=null)||(os.getSeguradora()==null&&linha.texto("seguradora")!=null)||(os.getCliente()==null&&linha.texto("cliente")!=null)||(os.getPlaca()==null&&linha.texto("placa")!=null)||(os.getDataHoraAtendimento()==null&&linha.dataHora("data_atendimento")!=null);return completa?AcaoLinhaPorto.ATUALIZAR:AcaoLinhaPorto.IGNORAR;}
    public AcaoLinhaPorto classificarOsComposicao(LinhaPorto linha,OrdemPagamentoPorto op,OrdemServicoPorto os){if(os==null)return AcaoLinhaPorto.IMPORTAR;if(os.getOrdemPagamento()!=null&&!os.getOrdemPagamento().getId().equals(op.getId()))return AcaoLinhaPorto.DIVERGENCIA;boolean conflito=diferentePreenchido(os.getValorTotal(),linha.decimal("valor_total"))||conflita(os.getEspecialidade(),linha.texto("especialidade"))||conflita(os.getSiglaViatura(),linha.texto("sigla_viatura"))||conflita(os.getSocorrista(),linha.texto("socorrista"))||conflita(os.getQra(),linha.texto("qra"));return conflito?AcaoLinhaPorto.DIVERGENCIA:AcaoLinhaPorto.ATUALIZAR;}
    ...
    @Transactional public OrdemPagamentoResponse criarOpManual(OrdemPagamentoRequest request,UsuarioPrincipal principal){if(Boolean.TRUE.equals(request.pagamentoConfirmado()))throw new IllegalArgumentException("Crie a OP como programada, importe sua composição e só então confirme o recebimento.");if(ops.findByNumero(request.numero().trim()).isPresent())throw new IllegalArgumentException("Já existe uma ordem de pagamento com este número.");
    ...
    private DadosOps carregarOps(){Map<Long,Composicao> composicoes=new HashMap<>();
        for(Object[] linha:oss.resumirPorOrdemPagamento())
            composicoes.put((Long)linha[0],new Composicao(((Number)linha[1]).longValue(),(BigDecimal)linha[2]));
        return new DadosOps(ops.findAllParaListagem(),composicoes);}
    /** Recorte por periodo aplicado no banco; sem periodo, cai na listagem completa. */
    private List<OrdemServicoPorto> carregarServicos(PortoOsFiltros f){
        if(f==null||f.dataInicio()==null||f.dataFim()==null)return oss.findAllParaListagem();
        return Boolean.TRUE.equals(f.porDataPagamento())?oss.findParaListagemPorPagamento(f.dataInicio(),f.dataFim())
            :oss.findParaListagemPorAtendimento(f.dataInicio(),f.dataFim());}
    private record Composicao(long quantidade,BigDecimal valor){static final Composicao VAZIA=new Composicao(0,BigDecimal.ZERO);}
    private record DadosOps(List<OrdemPagamentoPorto> ops,Map<Long,Composicao> composicoes){
        Composicao composicao(OrdemPagamentoPorto op){return composicoes().getOrDefault(op.getId(),Composicao.VAZIA);}}
    @Transactional(readOnly=true) public ResumoOrdensPagamentoResponse resumo(PortoFiltros filtros){return resumo(filtros,carregarOps());}
    private ResumoOrdensPagamentoResponse resumo(PortoFiltros filtros,DadosOps dados){List<OrdemPagamentoResponse> lista=listarOps(filtros,dados);LocalDate hoje=LocalDate.now();
    ...
    @Transactional(readOnly=true) public List<OrdemServicoResponse> listarOss(){return listarOss(new PortoOsFiltros(null,null,null,null,null,null,null,null,null,null,null,null,null));}
    ...
    private List<OrdemServicoResponse> listarOss(PortoOsFiltros filtros,DadosOps dados){PortoOsFiltros f=filtros==null?new PortoOsFiltros(null,null,null,null,null,null,null,null,null,null,null,null,null):filtros;
    ...
    @Transactional(readOnly=true) public List<PendenciaResponse> listarPendencias(){List<PendenciaResponse> r=new ArrayList<>();
        oss.findSemMotorista().forEach(x->r.add(new PendenciaResponse(null,"OS_SEM_SOCORRISTA",x.getId(),x.getNumero(),x.getValorTotal(),x.getDataAtendimento(),"ABERTA",
            motivoDaFaltaDeSocorrista(x.getQra()),x.getOrdemPagamento()==null?null:"OP "+x.getOrdemPagamento().getNumero(),null,null,x.getQra())));
    ...
    public static String motivoDaFaltaDeSocorrista(String qra){
        return qra==null||qra.isBlank()?"Relatório da Porto veio sem QRA nesta OS"
            :"QRA "+qra+" não está cadastrado em Equipe";
    }
    ...
    private OrdemPagamentoResponse op(OrdemPagamentoPorto x){List<OrdemServicoPorto> vinculadas=oss.findByOrdemPagamento(x);
        return op(x,new Composicao(vinculadas.size(),vinculadas.stream().map(OrdemServicoPorto::getValorTotal).filter(Objects::nonNull).reduce(BigDecimal.ZERO,BigDecimal::add)));}
    private OrdemPagamentoResponse op(OrdemPagamentoPorto x,Composicao composicao){BigDecimal soma=composicao.valor();BigDecimal diferenca=x.getValorTotal().subtract(soma);
    ...
    private void vincularMotoristaSeSeguro(OrdemServicoPorto os){Motorista encontrado=resolverMotorista.resolver(os);if(encontrado==null)os.limparVinculoAutomatico();else os.vincularMotoristaAutomaticamente(encontrado);}
