...
import com.anaiv.fluxogestao.financeiro.EnumsFinanceiros.StatusOperacionalPorto;
...
public class PortoRelatorioService {
    ...
    @Transactional(readOnly=true) public byte[] excelOss(PortoOsFiltros filtros){List<OrdemServicoResponse> oss=porto.listarOss(filtros);
        try(XSSFWorkbook wb=new XSSFWorkbook();ByteArrayOutputStream out=new ByteArrayOutputStream()){Estilos e=new Estilos(wb);Sheet s=wb.createSheet("Ordens de serviço");
            String[] h={"Número da OS","Data do atendimento","Especialidade","Nome no relatório Porto","QRA","Viatura","Placa","Cliente","Valor","Número da OP","Socorrista vinculado","Motivo da falta de vínculo"};cabecalho(s,e,h);int n=1;
            for(OrdemServicoResponse os:oss){Row r=s.createRow(n++);texto(r.createCell(0),os.numero(),e.texto);data(r.createCell(1),os.dataAtendimento(),e.data);texto(r.createCell(2),os.especialidade(),e.texto);texto(r.createCell(3),os.socorrista(),e.texto);texto(r.createCell(4),os.qra(),e.texto);texto(r.createCell(5),os.viatura(),e.texto);texto(r.createCell(6),os.placa(),e.texto);texto(r.createCell(7),os.cliente(),e.texto);numero(r.createCell(8),os.valorTotal(),e.moeda);texto(r.createCell(9),os.ordemPagamento(),e.texto);texto(r.createCell(10),os.motorista(),e.texto);texto(r.createCell(11),os.motorista()==null?PortoService.motivoDaFaltaDeSocorrista(os.qra()):null,e.texto);}
            total(s,e,n,8,"Total");finalizarTabela(s,h.length,n);wb.write(out);return out.toByteArray();
        }catch(Exception ex){throw new IllegalStateException("Não foi possível exportar as ordens de serviço Porto.",ex);}}
