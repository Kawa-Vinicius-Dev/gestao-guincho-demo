...
public class PortoController {
    ...
    @GetMapping("/ordens-servico/excel") public ResponseEntity<byte[]> excelOss(@ModelAttribute PortoOsFiltros filtros){return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION,"attachment; filename=ordens-servico-porto.xlsx").contentType(MediaType.parseMediaType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")).body(relatorios.excelOss(filtros));}
