...
export async function baixarRelatorioOpPorto(id:number,formato:'excel'|'pdf'){const token=tokenStorage.get();const response=await fetch(`/api/porto/ordens-pagamento/${id}/relatorios/${formato}`,{headers:token?{Authorization:`Bearer ${token}`}:{}});if(!response.ok)throw new ApiError('Não foi possível exportar a ordem de pagamento.',response.status);const blob=await response.blob(),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=`op-porto.${formato==='excel'?'xlsx':'pdf'}`;link.click();URL.revokeObjectURL(url)}
