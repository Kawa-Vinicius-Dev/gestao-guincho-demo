...
test('cria OP processada sem marcar como recebida',async()=>{
  ...
  const user=userEvent.setup({ delay: null });render(<MemoryRouter><PortoOrdensPagamentoPage/></MemoryRouter>);await screen.findByText('Nenhuma OP');await user.click(screen.getByRole('button',{name:/nova ordem de pagamento/i}));const dialogo=screen.getByRole('dialog')
...
test('edita uma OP manual e oferece os relatórios individuais',async()=>{
  ...
  const user=userEvent.setup({ delay: null });render(<MemoryRouter><PortoOrdensPagamentoPage/></MemoryRouter>);await user.click(await screen.findByRole('button',{name:'OP-EDIT-41'}))
...
test('lista serviço devolvido como pendência financeira',async()=>{
  ...
  render(<MemoryRouter><PortoPendenciasPage/></MemoryRouter>);expect(await screen.findByText('Serviço devolvido')).toBeInTheDocument();expect(screen.getByText('OS-200')).toBeInTheDocument();expect(screen.queryByText(/despesa/i)).not.toBeInTheDocument()
})
...
test('solicita Excel e PDF com os filtros ativos',async()=>{
  ...
  const user=userEvent.setup({ delay: null });render(<MemoryRouter><PortoOrdensPagamentoPage/></MemoryRouter>);await screen.findByText('Nenhuma OP')
...
test('mostra erro quando a exportação falha',async()=>{
  ...
  const user=userEvent.setup({ delay: null });render(<MemoryRouter><PortoOrdensPagamentoPage/></MemoryRouter>);await screen.findByText('Nenhuma OP');await user.click(screen.getByRole('button',{name:/exportar excel/i}))
...
test('abre a composição da OP e registra justificativa',async()=>{
  ...
  const user=userEvent.setup({ delay: null });render(<MemoryRouter><PortoOrdensPagamentoPage/></MemoryRouter>);await user.click(await screen.findByRole('button',{name:'OP-DET-91'}));expect(await screen.findByText('OS-DET-92')).toBeInTheDocument()
