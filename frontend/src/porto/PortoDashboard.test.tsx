...
test('separa realizado, programado e recebido no dashboard Porto',async()=>{
  let numeroOs='',visao=''
  servidor.use(http.get('/api/porto/dashboard',({request})=>{const params=new URL(request.url).searchParams;numeroOs=params.get('numeroOs')??'';visao=params.get('visao')??'';return HttpResponse.json({
  ...
  await user.click(screen.getByRole('button',{name:'Pagamentos'}));expect(visao).toBe('PAGAMENTOS')
