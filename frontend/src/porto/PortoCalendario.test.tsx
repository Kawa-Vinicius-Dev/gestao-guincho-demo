...
test('adiciona, edita e desativa uma data do calendário Porto',async()=>{
  let itens=[{id:1,dataPagamento:'2026-08-14',descricao:'Ciclo 14/08',ativo:true,criadoEm:'2026-01-01',atualizadoEm:'2026-01-01'}]
  servidor.use(
    http.get('/api/porto/calendario',()=>HttpResponse.json(itens)),
    http.post('/api/porto/calendario',async({request})=>{const body=await request.json() as {dataPagamento:string;descricao:string;ativo:boolean};const novo={id:2,...body,criadoEm:'2026-08-04',atualizadoEm:'2026-08-04'};itens=[...itens,novo];return HttpResponse.json(novo,{status:201})}),
    http.put('/api/porto/calendario/1',async({request})=>{const body=await request.json() as {dataPagamento:string;descricao:string;ativo:boolean};itens=itens.map(x=>x.id===1?{...x,...body}:x);return HttpResponse.json(itens[0])}),
  ...
  await user.type(screen.getByLabelText(/data de pagamento/i),'2026-09-16');await user.type(screen.getByLabelText(/descrição/i),'Ciclo 16/09');await user.click(screen.getByRole('button',{name:/adicionar data/i}))
