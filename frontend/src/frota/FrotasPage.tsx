...
export default function FrotasPage(){
  ...
  const resultadoFrota=frota.viaturas.reduce((s,v)=>s+v.resultado,0)
  const siglaDa=(v:Veiculo)=>(v.siglaPorto||v.identificacao).toUpperCase()
  ...
  return <div className="page-enter"><header className="page-heading"><div><span className="eyebrow">Ativos operacionais</span><h1>Viaturas</h1><p>Serviços, receitas e despesas de cada viatura no período.</p></div><div className="heading-actions"><button className="button button-primary" onClick={()=>setModal(true)}>+ Cadastrar viatura</button></div></header><section className="panel painel-filtros"><form className="ledger-filters" onSubmit={e=>e.preventDefault()}><SeletorPeriodo periodo={periodo} aoMudar={setPeriodo}/></form></section>
    ...
    {/* Topo da frota (Kawa, 24/09/2026): so o que se usa. "5/5 veiculos" e
        "melhor margem" eram enfeite e sairam. */}
    <GradeIndicadores>
      <Indicador rotulo="Serviços no período" valor={totalServicos===null?'—':numero(totalServicos)} link="/porto/ordens-servico" apoio="De todas as viaturas"/>
      <Indicador rotulo="Despesas das viaturas" valor={moeda(gastoFrota)} link="/lancamentos?atalho=DESPESAS" apoio="Pagas e ligadas a uma viatura"/>
      <Indicador rotulo="Resultado da frota" valor={moeda(resultadoFrota)} tom={resultadoFrota<0?'alerta':'neutro'}
        apoio={frota.despesasGerais?`Já descontados ${moeda(frota.despesasGerais)} de despesas gerais`:'Receita menos despesas das viaturas'}/>
    </GradeIndicadores>
    {veiculos.length?<section className="fleet-layout"><aside className="fleet-list" aria-label="Lista de veículos"><p className="fleet-list-titulo"><span>Viatura</span><span>Resultado</span></p>{veiculos.map(v=>{const r=financeiro?.resultadoPorVeiculo?.find(item=>item.veiculoId===v.id),saldo=r?.resultado??0;return <button key={v.id} className={v.id===selecionado?'active':''} onClick={()=>setSelecionado(v.id)}><span className="vehicle-monogram">{v.identificacao}</span><span className="fleet-item-nome"><strong>{v.modelo||v.identificacao}</strong><small>{ehAuxiliar(v.identificacao)?'Viatura auxiliar':v.placa??'Placa pendente'}{v.ativo?'':' · Inativo'}</small></span><span className="fleet-item-valor"><strong className={saldo>=0?'positive':'negative'}>{moeda(saldo)}</strong><small>{servicosDa(v)} {servicosDa(v)===1?'serviço':'serviços'}</small></span></button>})}</aside>
      ...
      <div><span className={`vehicle-status ${veiculo.ativo?'status-saudavel':'status-monitorar'}`}>{veiculo.ativo?'Ativo':'Inativo'}</span><button className="table-action" onClick={()=>{setEditando(veiculo);setModal(true)}}>Editar</button>{ehAuxiliar(veiculo.identificacao)?null:<button className="table-action table-action-danger" onClick={()=>setExcluindo(veiculo)}>Excluir</button>}</div></article>
        <div className="vehicle-metrics vehicle-metrics-linha"><article><span>Receita recebida</span><strong>{moeda(receitas)}</strong><small><Link to={`/lancamentos?atalho=RECEITAS&veiculo=${selecionado}`}>Ver no Extrato</Link></small></article><article><span>Despesas pagas</span><strong>{moeda(despesas)}</strong><small><Link to={`/lancamentos?atalho=DESPESAS&veiculo=${selecionado}`}>Ver no Extrato</Link></small></article><article className="focus"><span>Resultado</span><strong>{moeda(lucro)}</strong><small>{margem.toFixed(1)}% de margem</small></article>{aposRateio&&frota.despesasGerais?<article><span>Após despesas gerais</span><strong className={aposRateio.resultado<0?'negative':undefined}>{moeda(aposRateio.resultado)}</strong><small>{moeda(aposRateio.rateio)} de aluguel, contador etc.</small></article>:null}</div>
        <div className="vehicle-metrics vehicle-metrics-linha"><article><span>Serviços</span><strong>{numero(servicosDa(veiculo))}</strong><small><Link to={`/porto/ordens-servico?sigla=${encodeURIComponent(siglaDa(veiculo))}`}>Ver as OS da viatura</Link></small></article>{aguardando&&aguardando.total?<article><span>Aguardando OP</span><strong>{aguardando.total}</strong><small><Link to={`/porto/ordens-servico?situacao=AGUARDANDO&competencia=1&sigla=${encodeURIComponent(siglaDa(veiculo))}`}>{moeda(aguardando.previsto)} previstos{aguardando.semValor?` · ${aguardando.semValor} sem valor`:''}</Link></small></article>:null}<article><span>Km morto</span><strong>{numero(resultado?.kmMorto??0)} km</strong><small><Link to="/quilometragem">{moeda(resultado?.custoKmMorto??0)} improdutivos</Link></small></article></div>
        {(()=>{
