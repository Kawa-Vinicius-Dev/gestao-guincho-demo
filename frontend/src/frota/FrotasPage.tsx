...
export default function FrotasPage(){
  ...
  async function salvar(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);setSalvando(true);setMensagem('')
    const dados={identificacao:String(f.get('identificacao')),placa:String(f.get('placa')),modelo:String(f.get('modelo')||''),custoPorKm:Number(f.get('custoPorKm')),siglaPorto:String(f.get('siglaPorto')||'').trim()||null}
    try{
  ...
  return <div className="page-enter"><header className="page-heading"><div><span className="eyebrow">Ativos operacionais</span><h1>Veículos e custos</h1><p>Receitas, despesas e eficiência calculadas a partir dos vínculos reais do PostgreSQL.</p></div><div className="heading-actions"><label className="month-picker"><span>Competência</span><input type="month" value={mes} onChange={e=>setMes(e.target.value)}/></label><button className="button button-primary" onClick={()=>setModal(true)}>+ Cadastrar veículo</button></div></header>
    ...
    {modal?<Modal etiqueta="Cadastro oficial" titulo="Novo veículo" aoFechar={()=>setModal(false)}><form onSubmit={salvar} className="form-grid two-columns"><label className="field"><span>Identificador</span><input name="identificacao" defaultValue={editando?.identificacao??''} required/></label><CampoPlaca rotulo="Placa" name="placa" defaultValue={editando?.placa} required/><label className="field field-wide"><span>Modelo</span><input name="modelo" defaultValue={editando?.modelo??''}/></label><CampoValor rotulo="Custo por km" name="custoPorKm" defaultValue={editando?.custoPorKm} exigirPositivo={false} required
          ajuda="Quanto este veículo custa por quilômetro rodado. É o que transforma km morto em dinheiro no dashboard."/>
        <label className="field"><span>Sigla na Porto</span><input name="siglaPorto" defaultValue={editando?.siglaPorto??''} placeholder="L25" autoCapitalize="characters" autoCorrect="off" spellCheck={false}/><small>Como a Porto chama esta viatura no painel do dia. É o que liga o serviço importado a este veículo.</small></label>
