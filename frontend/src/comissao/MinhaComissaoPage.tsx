...
export default function MinhaComissaoPage(){
  ...
  useEffect(()=>{listarPeriodosComissoes().then(lista=>{setPeriodos(lista);const atual=periodoCorrente(lista);if(atual)setPeriodoId(atual.id);if(!atual)setCarregando(false)}).catch(e=>{setErro(e.message);setCarregando(false)})},[])
  const [carregando,setCarregando]=useState(true)
  useEffect(()=>{if(!periodoId)return;setCarregando(true);lerComissaoDoCiclo(periodoId).then(setComissao).catch(e=>setErro(e.message)).finally(()=>setCarregando(false))},[periodoId])
