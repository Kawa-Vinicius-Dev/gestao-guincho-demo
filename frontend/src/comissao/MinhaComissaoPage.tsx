...
import { lerComissaoDaOp, listarOpsComissao, registrarAlimentacao } from '../dados/comissoes'
import type { Comissao,OrdemPagamentoPorto } from '../types/modelos'
...
export default function MinhaComissaoPage(){
  ...
  return <div className="page-enter commission-page"><header className="page-heading"><div><span className="eyebrow">Área do socorrista</span><h1>Minha comissão</h1><p>Acompanhe somente seus serviços pagos e sua alimentação no fechamento Porto.</p></div><Selecao rotulo="Ordem de pagamento" className="month-picker" vazio="Selecione" value={periodoId||''}
      onChange={e=>setPeriodoId(Number(e.target.value))}
      opcoes={periodos.map(p=>({valor:p.id,texto:rotuloOp(p)}))}/></header>
