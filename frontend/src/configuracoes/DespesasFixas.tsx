...
export function DespesasFixas() {
  ...
  return <section className="panel settings-card settings-card-largo" aria-label="Despesas fixas">
    <header className="settings-card-cabecalho">
      ...
      <button className="button button-primary" onClick={() => { setErro(''); setEditando('nova') }}>+ Nova despesa fixa</button>
    </header>
    ...
    {fixas.length ? <>
      ...
      <div className="table-scroll"><table>
        <thead><tr><th>Descrição</th><th>Categoria</th><th>Todo dia</th><th>Parcelas</th><th className="th-numero">Valor</th><th/></tr></thead>
        <tbody>{fixas.map(f => <tr key={f.id} className={f.ativo ? undefined : 'linha-inativa'}>
          ...
          <td className="col-acoes"><span className="acoes-da-linha">
            <button className="table-action" onClick={() => { setErro(''); setEditando(f) }}>Editar</button>
