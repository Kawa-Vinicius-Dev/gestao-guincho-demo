...
import { avaliarImportacaoPortoPorNumero, cancelarImportacaoPorto, confirmarImportacaoPorto, confirmarImportacaoPortoPorNumero, criarPreviaConteudoPorto, criarPreviaPorto, listarCalendarioPorto } from '../api/porto'
...
export default function PortoImportacoesPage(){
  ...
  const [mensagem,setMensagem]=useState(''),[erro,setErro]=useState(''),[carregando,setCarregando]=useState(false),[inputKey,setInputKey]=useState(0)
  ...
  async function analisar(){
    ...
    setCarregando(true);setErro('');setNumeroOp('');setPeriodoId('');limparConfirmacoes()
  ...
  async function avaliarNumero(valor=numeroOp){
    const numero=valor.trim();if(!previa||!numero||!previa.requerOrdemPagamento)return
    setCarregando(true);setErro('');limparConfirmacoes()
    try{setPrevia(await avaliarImportacaoPortoPorNumero(previa.id,numero))}catch(e){setErro((e as Error).message)}finally{setCarregando(false)}
  }
  function alterarNumero(valor:string){setNumeroOp(valor);limparConfirmacoes()}
  async function confirmar(){
    if(!previa||previa.requerOrdemPagamento&&(!numeroOp.trim()||!periodoId))return
    setCarregando(true);setErro('')
    try{
      const r=previa.requerOrdemPagamento
        ?await confirmarImportacaoPortoPorNumero(previa.id,{numeroOrdemPagamento:numeroOp.trim(),calendarioPagamentoId:Number(periodoId),confirmarDivergencias,confirmarReassociacoes,motivoDivergencia:motivoDivergencia||undefined,justificativaDivergencia:justificativaDivergencia.trim()||undefined})
        :await confirmarImportacaoPorto(previa.id,undefined,confirmarDivergencias)
      ...
      setPrevia(null);setArquivo(null);setNumeroOp('');setPeriodoId('');limparConfirmacoes();setInputKey(x=>x+1)
    }catch(e){setErro((e as Error).message)}finally{setCarregando(false)}
  }
  async function cancelar(){
    if(!previa)return;setCarregando(true);setErro('')
    try{await cancelarImportacaoPorto(previa.id);setMensagem('Prévia cancelada. Corrija e reenvie o arquivo quando estiver pronto.');setPrevia(null);setArquivo(null);setNumeroOp('');setPeriodoId('');limparConfirmacoes();setInputKey(x=>x+1)}catch(e){setErro((e as Error).message)}finally{setCarregando(false)}
  }
  function limpar(){setConteudo('');setArquivo(null);setPrevia(null);setErro('');setMensagem('');setNumeroOp('');setPeriodoId('');limparConfirmacoes();setInputKey(x=>x+1)}

  const temErros=Boolean(previa?.erros.length||previa?.linhas.some(l=>l.acao==='ERRO'))
  const analise=previa?.analiseOrdemPagamento?.numero===numeroOp.trim()?previa.analiseOrdemPagamento:undefined
    ...
    <section className="panel porto-import-card"><div className="segmented porto-import-modes" role="group" aria-label="Forma de importação"><button className={modo==='arquivo'?'active':''} onClick={()=>{setModo('arquivo');setPrevia(null)}}>Enviar arquivo</button><button className={modo==='colagem'?'active':''} onClick={()=>{setModo('colagem');setPrevia(null)}}>Colar serviços da Porto</button></div>
      ...
      {previa?<div className="porto-preview"><header className="panel-title"><div><span className="eyebrow">Prévia detectada</span><h2>{rotulos[previa.tipo]}</h2></div><span className="import-pill">{previa.totalLinhas} linhas</span></header>
        ...
        <footer className="porto-confirm porto-confirm-sticky" aria-label="Ações da prévia">
          ...
          {previa.requerOrdemPagamento?<label className="field"><span>Número da OP</span><input aria-label="Número da OP" value={numeroOp} onChange={e=>alterarNumero(e.target.value)} onBlur={()=>void avaliarNumero()} required placeholder="Ex.: 06422281"/></label>:null}
          {previa.requerOrdemPagamento?<label className="field"><span>Período financeiro</span><select aria-label="Período financeiro" value={periodoId} onChange={e=>{setPeriodoId(e.target.value);if(numeroOp.trim()&&!analise)void avaliarNumero()}} required><option value="">Selecione o período</option>{periodos.filter(p=>p.ativo||String(p.id)===periodoId).map(p=><option key={p.id} value={p.id}>{p.descricao} · {dataBr(p.competenciaInicio)} a {dataBr(p.competenciaFim)}</option>)}</select></label>:null}
          ...
          <button className="button button-primary" disabled={carregando||temErros||!divergenciaConfirmada||temReassociacoes&&!confirmarReassociacoes||previa.requerOrdemPagamento&&(!numeroOp.trim()||!periodoId||!analise)||previa.linhas.length===0} onClick={confirmar}>Confirmar importação</button>
        </footer>
