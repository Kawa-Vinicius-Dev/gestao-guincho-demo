...
export default function EscalaPage() {
  ...
  return <div className="page-enter">
    <header className="page-heading"><div><span className="eyebrow">Semana de 20 a 26 de julho</span><h1>Escala operacional</h1><p>Uma visão simples da cobertura por socorrista, veículo e turno.</p></div><button className="button button-ghost" onClick={() => window.print()}>Imprimir escala</button></header>
    ...
    <section className="panel schedule-table"><div className="table-scroll"><table><thead><tr><th>Dia</th><th>Socorrista</th><th>Veículo</th><th>Turno</th><th>Status</th></tr></thead>
      <tbody>{state.escala.map(item => <tr key={item.id}><td><strong>{data(item.dia)}</strong><small>{new Intl.DateTimeFormat('pt-BR', { weekday: 'long' }).format(new Date(`${item.dia}T12:00:00`))}</small></td>
        <td><strong>{state.socorristas.find(socorrista => socorrista.id === item.socorristaId)?.nome}</strong></td>
