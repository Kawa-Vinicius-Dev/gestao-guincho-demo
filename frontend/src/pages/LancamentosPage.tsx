...
import type { ClasseCusto, OrigemLancamentoDemo, TipoLancamento } from '../demo/modelosDemo'
...
export default function LancamentosPage({ filtroInicial = '' }: { filtroInicial?: '' | TipoLancamento }) {
  ...
  function salvar(evento: FormEvent<HTMLFormElement>) {
    ...
    adicionarLancamento({
      ...
      origem: String(formulario.get('origem')) as OrigemLancamentoDemo,
      classeCusto,
  ...
  return <div className="page-enter">
    <header className="page-heading"><div><span className="eyebrow">{tipoFixo === 'RECEITA' ? 'Entradas' : tipoFixo === 'DESPESA' ? 'Saídas' : 'Financeiro operacional'}</span>
      <h1>{tipoFixo === 'RECEITA' ? 'Receitas' : tipoFixo === 'DESPESA' ? 'Despesas' : 'Lançamentos'}</h1>
      ...
      <div className="heading-total-with-action"><span><small>Saldo filtrado</small><strong className={total >= 0 ? 'positive' : 'negative'}>{moeda(total)}</strong></span>
        <button className="button button-primary" onClick={() => setModal(true)}>+ {tipoFixo === 'DESPESA' ? 'Registrar despesa' : tipoFixo === 'RECEITA' ? 'Nova receita' : 'Novo lançamento'}</button></div>
    </header>
    ...
    <section className="panel">
      ...
      <div className="table-scroll"><table><thead><tr><th>Data</th><th>Lançamento</th><th>Categoria</th><th>Veículo</th><th>Origem</th><th>Status</th><th>Valor</th><th/></tr></thead>
        <tbody>{lista.map(item => {
          ...
          return <tr key={item.id}><td>{data(item.data)}</td><td><strong>{item.descricao}</strong><small>{item.protocolo ?? (item.tipo === 'RECEITA' ? 'Entrada' : item.classeCusto === 'FIXO' ? 'Custo fixo' : 'Custo variável')}</small></td>
            <td>{item.categoria}</td><td>{veiculo?.codigo ?? '—'}</td><td>{item.origem}</td><td><span className={`ledger-status ledger-${item.status.toLowerCase()}`}>{item.status.replace('_', ' ')}</span></td>
    ...
    {modal ? <div className="modal-backdrop"><section className="modal modal-wide" role="dialog" aria-modal="true" aria-labelledby="titulo-lancamento"><header><div><span className="eyebrow">Novo registro financeiro</span><h2 id="titulo-lancamento">{funcionario ? 'Registrar despesa' : 'Novo lançamento'}</h2></div><button aria-label="Fechar" onClick={() => setModal(false)}>×</button></header>
      <form onSubmit={salvar} className="form-grid three-columns">
        ...
        <label className="field two-span"><span>Descrição</span><input name="descricao" placeholder="Ex.: atendimento Porto Seguro lote 092" required/></label>
        ...
        <label className="field"><span>Status</span><select name="status" disabled={funcionario}>{tipoFormulario === 'RECEITA' ? <><option value="RECEBIDO">Recebido</option><option value="A_RECEBER">A receber</option></> : <><option value="PAGO">Pago</option><option value="PENDENTE">Pendente</option></>}</select></label>
        ...
        <label className="field"><span>Funcionário</span><select name="funcionarioId"><option value="">Não vinculado</option>{state.funcionarios.map(item => <option value={item.id} key={item.id}>{item.nome}</option>)}</select></label>
        <label className="field"><span>Origem</span><select name="origem"><option>Manual</option><option>Banco</option><option>Porto Seguro</option><option>Outra seguradora</option><option>Excel</option></select></label>
        <label className="field"><span>Protocolo</span><input name="protocolo" placeholder="Opcional"/></label>
