...
test('comissao: o administrador consulta a de um socorrista', async () => {
  let corpo: unknown = null
  servidor.use(http.post(`${URL_SUPABASE}/rest/v1/rpc/comissao_do_ciclo`, async ({ request }) => {
    corpo = await request.json()
    return HttpResponse.json({ comissaoBruta: 200, liquido: 150, servicos: [] })
  }))
  const { lerComissaoDoCiclo } = await carregar('auth,comissoes', () => import('./comissoes'))

  const comissao = await lerComissaoDoCiclo(1, 7)

  expect(corpo).toEqual({ p_calendario_id: 1, p_motorista_id: 7 })
