...
test('administrador abre o socorrista pela Equipe e consulta composição oficial e período anterior',async()=>{
  ...
  const total=screen.getByText('Total de serviços prestados').closest<HTMLElement>('.metric')
  const pagos=screen.getByText('Serviços já pagos').closest<HTMLElement>('.metric')
  ...
  expect(within(within(resumo).getByText('Produção paga').closest<HTMLElement>('.metric')!).getByText('R$ 500,00')).toBeInTheDocument()
  expect(within(within(resumo).getByText('Comissão', { selector: 'span' }).closest<HTMLElement>('.metric')!).getByText('R$ 100,00')).toBeInTheDocument()
  expect(within(within(resumo).getByText('Descontos').closest<HTMLElement>('.metric')!).getByText('R$ 30,00')).toBeInTheDocument()
  expect(within(within(resumo).getByText('Líquido').closest<HTMLElement>('.metric')!).getByText('R$ 70,00')).toBeInTheDocument()
