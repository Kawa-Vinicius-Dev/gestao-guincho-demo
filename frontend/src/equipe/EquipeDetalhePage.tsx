...
export default function EquipeDetalhePage(){
  ...
  function pedirComissao(servico:{id:number;numeroOs:string;valorServico:number;comissaoGerada?:number;semComissao?:boolean}){
    ...
    setPedido({titulo:tirando?'Cancelar esta OS?':'Reativar esta OS?',
      efeito:tirando
        ?<>A OS deixa de contar para <strong>{detalhe?.nome}</strong> e não gera comissão. Se ela já foi paga, a comissão é recalculada.</>
        :<>A OS volta a contar para <strong>{detalhe?.nome}</strong> e gera comissão de novo.</>,
      resumo:[
        [tirando?'Comissão que sai':'Comissão que volta',emJogo??'Ainda sem valor'],
        ['OS',servico.numeroOs],
        ['Valor do serviço',moeda(servico.valorServico)],
      ],
      textoConfirmar:tirando?'Cancelar OS':'Reativar OS',perigo:tirando,
  ...
  return <div className="page-enter employee-detail-page">
    ...
    {detalhe?<>
      ...
      <section className="metric-grid employee-summary" aria-label="Resumo do período">
        {/* Cada cartao leva ao que ele resume, como no resto do sistema (Kawa, 23/09/2026). */}
        <a className="metric metric-link" href="#servicos-do-periodo"><span>Total de serviços prestados</span><strong>{detalhe.totalServicosPrestados}</strong><small>Inclui OS ainda não pagas</small></a>
        <a className="metric metric-link" href="#servicos-do-periodo"><span>Serviços já pagos</span><strong>{detalhe.comissao.quantidadeServicosPagos}</strong><small>Somente OS já pagas numa OP</small></a>
        <a className="metric metric-link" href="#servicos-do-periodo"><span>Produção paga</span><strong>{moeda(detalhe.comissao.producaoPaga)}</strong><small>Ver os serviços</small></a>
        <Link className="metric metric-focus metric-link" to="/comissoes"><span>Comissão</span><strong>{moeda(detalhe.comissao.comissaoBruta)}</strong><small>Ver comissões da equipe</small></Link>
        <a className="metric metric-link" href="#descontos-da-comissao"><span>Descontos</span><strong>{moeda(detalhe.comissao.descontos)}</strong><small>{moeda(detalhe.comissao.descontosPendentes)} aguardando aprovação</small></a>
