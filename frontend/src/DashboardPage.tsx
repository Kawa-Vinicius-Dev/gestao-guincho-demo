...
export default function DashboardPage(){
  ...
  return <div className="page-enter dashboard-tech">
    ...
    {financeiro?<><section className="finance-lane" aria-label="Fluxo do resultado operacional"><div><span>Receita do mês</span><strong>{moeda(financeiro.receitaRecebida)}</strong><small>Recebimentos confirmados no financeiro</small></div><i className="lane-separator">−</i><div><span>Despesas do mês</span><strong>{moeda(financeiro.despesasPagas)}</strong><small>{financeiro.receitaRecebida?((financeiro.despesasPagas/financeiro.receitaRecebida)*100).toFixed(1):0}% da receita</small></div><i className="lane-separator">=</i><div className="lane-result"><span>Lucro operacional</span><strong>{moeda(financeiro.saldoRealizado)}</strong><small>Margem de {margem.toFixed(1)}%</small></div></section>
      ...
      <section className="panel"><header className="panel-title"><div><span className="eyebrow">Por pessoa</span><h2>Faturamento e custo por socorrista</h2></div><Link to="/equipe">Abrir socorristas</Link></header>
        <FaturamentoECusto descricao="Faturamento e custo por socorrista" vazio="Nenhum serviço pago com socorrista vinculado neste período." linhas={(financeiro.resultadoPorSocorrista??[]).map(p=>({id:p.motoristaId,rotulo:p.socorrista,faturamento:p.producao,custo:p.custoTotal}))}/>
        {financeiro.resultadoPorSocorrista?.length?<div className="table-scroll"><table><thead><tr><th>Socorrista</th><th>Serviços</th><th>Produção</th><th>Comissão</th><th>Despesas próprias</th><th>Custo total</th></tr></thead>
      ...
      <section className="panel vehicle-results vehicle-results-v2"><header className="panel-title"><div><span className="eyebrow">Por viatura</span><h2>Faturamento e custo por veículo</h2></div><Link to="/veiculos">Abrir veículos</Link></header><FaturamentoECusto descricao="Faturamento e custo por veículo" vazio="Nenhum resultado por veículo no período." linhas={financeiro.resultadoPorVeiculo.map(v=>({id:v.veiculoId,rotulo:v.veiculo,faturamento:v.receitas,custo:v.despesas}))}/>{financeiro.resultadoPorVeiculo.length?<div className="table-scroll"><table><thead><tr><th>Veículo</th><th>Faturamento</th><th>Custo</th><th>Resultado</th><th>Km morto</th><th>Custo km morto</th></tr></thead><tbody>{financeiro.resultadoPorVeiculo.map(item=><tr key={item.veiculoId}><td><strong>{item.veiculo}</strong></td><td>{moeda(item.receitas)}</td><td>{moeda(item.despesas)}</td><td className={item.resultado>=0?'positive':'negative'}><strong>{moeda(item.resultado)}</strong></td><td>{numero(item.kmMorto)} km</td><td>{moeda(item.custoKmMorto)}</td></tr>)}</tbody></table></div>:null}</section></>:<div className="loading-card">Carregando indicadores financeiros oficiais…</div>}
