...
test('edita receita manual sem duplicar e exclui somente após a confirmação',async()=>{
  ...
  await user.click(within(manual).getByRole('button',{name:/editar/i}));const dialogo=screen.getByRole('dialog');const valor=within(dialogo).getByLabelText(/^valor$/i);await user.clear(valor);await user.type(valor,'17500');await user.click(within(dialogo).getByRole('button',{name:/salvar alterações/i}));expect(await screen.findByText('R$ 175,00')).toBeInTheDocument();expect(screen.getAllByRole('row',{name:/manual/i})).toHaveLength(1)
